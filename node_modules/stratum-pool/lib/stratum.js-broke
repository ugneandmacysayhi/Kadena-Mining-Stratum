...
var StratumClient = function (options) {
  ...
  if (this.socket.localPort == 3701 || this.socket.localPort == 3702)
    this.f1 = true;
  else
    this.f1 = false;
...
*/
  ...
  this.sendDifficulty = function (difficulty) {
    ...
    let tgtPadded = BigInt("0x0000000000ffffffffffffffffffffffffffffffffffffffffffffffffffffff");
    //let diff = util.target_to_diff(tgtPadded);
    //console.log("test is " + diff);
  ...
  this.sendMiningJob = function (blockTemplate) {
    ...
    if (options.socket.localPort == 3701 || options.socket.localPort == 3702) {
    ...
    // Test reconnection logic for F1
    if (_this.f1 && lastActivityAgo > 300 * 1000) {
      _this.emit('socketTimeout', 'F1 last submitted a share was ' + ((lastActivityAgo / 1000) | 0) + ' seconds ago');
      _this.socket.destroy();
      return;
    } else if (lastActivityAgo > options.connectionTimeout * 1000) {
