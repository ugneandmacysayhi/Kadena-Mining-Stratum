...
var NoncePrefixCounter = function () {
  ...
  return {
    next: function () {
      ...
      let prefix = util.toLeftPaddedHexString(count.toString(16), 4);
      console.log(`noncePrefix count ${count} is ${prefix}`);
      return prefix;
    },
  };
};
...
var StratumClient = function (options) {
  ...
  // f1 todo change to 3701
  if (this.socket.localPort == 3701)
    this.f1 = true;
  else
    this.f1 = false;
...
*/
  function handleSubscribe(message) {
    ...
    _this.emit('subscription', {}, function (error, data) {
      ...
      const noncePrefixPadded = _this.noncePrefix;
...
*/
  function handleAuthorize(message, replyToSocket) {
    ...
    options.authorizeFn(_this.remoteAddress, _this.minerVersion, options.socket.localPort, _this.workerName, _this.workerPass, function (result) {
      ...
      //console.log("stratum.js handleAuthorize uuid is  " + _this.uuid);
      ...
      if (typeof _this.ports[options.socket.localPort] !== 'undefined' && _this.ports[options.socket.localPort].diff) {
        ...
        let tgtHex = "0x" + util.diff_to_target(BigIntDiff).toString(16);
        //let tgtHex = "0000000000ffffffffffffffffffffffffffffffffffffffffffffffffffffff";
        if (_this.f1) {
          //tgtHex = "0000000000ffffffffffffffffffffffffffffffffffffffffffffffffffffff";
          tgtHex = "0x" + util.diff_to_target(BigIntDiff).toString(16);
        }
  ...
  function setupSocket() {
    ...
    socket.on('close', function () {
      //console.log("stratum/js socket on close. uuid is " + this.uuid)
  ...
  this.sendDifficulty = function (difficulty) {
    ...
    let target = util.padLeftHexString(
      util.diff_to_target(BigIntDiff).toString(16),
      64
    );

    console.log("***********sendDifficulty() difficulty=" + difficulty + " target " + target);

    //let tgtPadded = "0000000000ffffffffffffffffffffffffffffffffffffffffffffffffffffff";
    // if (_this.f1) {
    //   // target = util.padLeftHexString(
    //   //   util.diff_to_target(BigIntDiff).toString(16),
    //   //   64
    //   // );
    //   tgtPadded = util.toLeftPaddedHexString(target.toString(16), 64);

    //   //tgtPadded = "0000000000ffffffffffffffffffffffffffffffffffffffffffffffffffffff";
    // }
    //else
    //tgtPadded = util.toLeftPaddedHexString(target.toString(16), 64);
  ...
  this.sendMiningJob = function (blockTemplate) {
    ...
    // todo change to 3701 for f1
    // if (options.socket.localPort == 3701) {
    //   //console.log("BlackMiner atlernative job format");
    //   //const timestamp = Math.floor(Date.now() / 1000);
    //   //const jobParams = blockTemplate.getjobParams();
    //   const longChainId = blockTemplate.longChainId;

    //   const diff = '000000ffffffffffffffffffffffffffffffffffffffffffffffffffffffffff';
    //   //console.log("jobParams is " + JSON.stringify(jobParams));
    //   const params = [
    //     `${jobParams[0]}`,
    //     longChainId,
    //     diff,
    ...
    blockTemplate.startTime = Date.now();
    ...
    //console.log(chalk.green(`sendMiningJob chain `) + chalk.gray(`${blockTemplate.chainId}:`) + chalk.yellow(`${blockTemplate.jobId} longChainId ${longChainId}`));
    sendJson(json);
  ...
  this.broadcastMiningJobs = function (chainId, jobManager) {
    ...
    for (var clientId in stratumClients) {
      ...
      if (client.chainId === null) {
        ...
        console.log(`stratum sending job chainid ${job.chainId} height ${job.height}`);
      ...
      if (client.chainId == chainId) {
        ...
        console.log(chalk.rgb(255, 0, 255)(`(BROADCAST) MATCHED CLIENT ON CHAIN  client chainid ${client.chainId} job chainid ${job.chainId} height ${job.height}`));
