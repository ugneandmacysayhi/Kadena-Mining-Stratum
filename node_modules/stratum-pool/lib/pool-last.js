...
let cutPath = "/chainweb/0.0/mainnet01/cut";
let chainBytes = [
  ...
  Buffer.from('0B000000', 'hex'),
  Buffer.from('0C000000', 'hex'),
  Buffer.from('0D000000', 'hex'),
  Buffer.from('0E000000', 'hex'),
  Buffer.from('0F000000', 'hex'),
  Buffer.from('11000000', 'hex'),
  Buffer.from('12000000', 'hex'),
  Buffer.from('13000000', 'hex'),
  Buffer.from('14000000', 'hex'),
];
...
let cutNodes = [
  ...
  //  `us-w3.chainweb.com`,
  `jp1.chainweb.com`,
  //  `cnhzh-node00.kadena.fish`, // f2pool
];
...
var pool = (module.exports = function pool(options, authorizeFn) {
          ...
          for (let i = 0; i < CHAINS; i++) {
            ...
            if (job) {
              if (job.height == hashes[i].height) {
                ...
                GetBlockTemplate(i, 0, function (error, result) {
                  if (error) {
  ...
  function OutputPoolInfo() {
    ...
    /*  
    let rpcData = _this.apiClient.getCut2().then(body => {
      //console.log(`body ${body}`)
      _this.jobManager.processTemplate(body);
    });
*/
    //console.log("rpcData TEST:" + u2.inspect(rpcData));
  ...
  function SubmitBlock(blockHex) {
    ...
    try {
      ...
      options.url = 'https://' + nodes[blockHex.nodeIndex].hostname + ':' + port + process.env.SOLVED;
    ...
    let chainId = blockHex.client.chainId;
    //console.log(chalk.red(`${Date.now()} SubmitBlock Manually INVALIDATE chainId ${chainId}`));
      ...
      .on('client.connected', function (client) {
        ...
        client
          .on('clientReady', function (client) {
            ...
            let randChainId = Math.floor(Math.random(10) * CHAINS);
          ...
          .on('socketDisconnect', function (iceWorkerId) {
            _this.emit('minerDisconnect', iceWorkerId);
  ...
  /* This is the new endpoint we're waiting on
  function SetupBlockPollingPush() {

    console.log(`SetupBlockPolling()`);

    const js = JSON.stringify({
      chain: 1,
      miner: process.env.PUBKEY
    });

   // let path = "/mining/stream";
    let nodes = ["test.icemining.ca"];
    ...
    // for (let i = 0; i < nodes.length; i++) {
      const options = {
        ...
        timeout: 1000,
        path: "/chainweb/0.0/mainnet01/mining/stream",
        method: "GET",
        headers: {
      ...
      const req = https.request(options, res => {
        ...
        if (res.statusCode != 200) {
          ...
          //   `Error getting work from node: ${res.statusCode}`
          // );
          return;
        }
        res.on("data", d => {
          ...
          //const height = util.getHeight(rpcData);
            //console.log(chalk.red(`*** chainId: ${chainId} height ${height}`));
 ...
 */
  ...
  function invalidateChainAndBroadcast(chainId) {
    ...
    if (_this.jobManager.getJobByChainId(chainId) === null) return;
  ...
  function retriableRequest(options, chainId, nodeIndex) {
    ...
    const req = https.request(options, res => {
      ...
      res.on('data', d => {
        //let cut = Buffer.from(d, 'binary').toString('hex');
        //chainId = util.getChainId(cut);
        //console.log(chalk.red(`${Date.now()} USING MINING/UPDATES TO INVALIDATE chainId ${chainId}`));
        //invalidateChainAndBroadcast(chainId);
        ...
        GetBlockTemplate(chainId, nodeIndex, function (error, result) {
          if (error) {
            console.log(`ERROR IN GBT`);
          }
          // if (result) {
    ...
    req.on('timeout', () => {
      ...
      req.abort();
      retriableRequest(options, chainId, nodeIndex);
    });
    req.on('error', error => {
  ...
  function SetupGBTBlockPolling() {
    ...
    blockPollingIntervalId = setInterval(function () {
      ...
      for (let node of nodes) {
        ...
        GetBlockTemplate('any', node.nodeIndex, function (error, result) {
          if (error) {
  ...
  function SetupBlockPolling() {
    ...
    for (let node of nodes) {
      ...
      for (let chainId = 0; chainId < CHAINS; chainId++) {
        retriableRequest(options, chainId, node.nodeIndex);
      }
    }
  }
  ...
  /*
    ...
    */
  function GetBlockTemplate(chainId, nodeIndex, callback) {
    ...
    let path = process.env.WORK;
    if (chainId === 'any') path = process.env.WORK;

    //let hostnames = ['test.icemining.ca'];
    ...
    const req = https.request(options, res => {
      ...
      res.on('data', d => {
        ...
        const chainId = util.getChainId(rpcData);
        const height = util.getHeight(rpcData);
        var processedNewBlock = _this.jobManager.processTemplate(rpcData, chainId, nodeIndex, nodeId);
        callback(null, {}, rpcData);
      });
    });
    req.on('error', error => {
